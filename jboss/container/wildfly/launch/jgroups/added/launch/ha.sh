...
"
    ...
    if [ ${result} -ne 0 ]; then
      echo "You have set JGROUPS_CLUSTER_PASSWORD environment variable to configure AUTH protocol but your configuration does not contain any stacks in the JGroups subsystem. Fix your configuration." >> "${CLI_SCRIPT_ERROR_FILE}"
    else
      ...
      while read -r stack; do
        ...
        if [ ${result} -eq 0 ]; then
          ...
          if [ ${index} -eq ${#arr[@]} ]; then
            echo "You have set JGROUPS_CLUSTER_PASSWORD environment variable to configure AUTH protocol but GMS protocol was not found for ${stack^^} stack. Fix your configuration to contain the pbcast.GMS protocol in the JGroups subsystem for this to happen." >> "${CLI_SCRIPT_ERROR_FILE}"
            missingGMS="true"
            continue
          fi
        fi
